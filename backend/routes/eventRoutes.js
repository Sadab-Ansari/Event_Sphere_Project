...
const {
  ...
  sendEmailToOrganizer, // Fixed import
...
router.put("/update/:eventId", authMiddleware, (req, res, next) => {
  upload(req, res, (err) => {
    ...
    if (!req.file) {
      return updateEvent(req, res, next); // Continue without banner update
    }
...
// Send Email to Organizer (Only logged-in users can send an email)
router.post("/send-email/:eventId", authMiddleware, sendEmailToOrganizer);
